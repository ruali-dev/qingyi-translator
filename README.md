...
- **自由调整窗口**：拖动卡片顶部移动位置，拖动右下角调整阅读尺寸。
- **论文公式渲染**：离线显示 LaTeX 分式、积分、矩阵等公式，复制时保留源码；支持 `$…$`、`$$…$$`、`\(…\)`、`\[…\]` 和常见公式环境。
...
- 公式支持常见 LaTeX 数学语法，不支持的命令会尽可能保留源码；无法从 PDF 文本选区恢复缺失的公式结构，自定义宏和完整 LaTeX 文档不在支持范围内。
